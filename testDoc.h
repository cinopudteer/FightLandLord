...
public:
	virtual ~CtestDoc();
	std::vector<CCard*> tempDeck;           //中间没有发牌的牌堆
...
=======
	...
	CCard* FindAndRemoveCardFromHand(int& playerIndex, CardData& cards);//讲手牌移到打出的牌
	BOOL InitializeNetwork(BOOL isServer, CString ipAddress,UINT port);//开始网络连接
	void SetSetupDialog(Login* pDlg); // << 新增 >>
	void OnClientConnected(ClientSocket* pClient);
	BOOL FindCard(const CardData& data, CCard*& pCard);//根据网络传输的牌找到本地的牌
	...
	int m_connectedClients; //服务器用于计数已连接的客户端
	int m_nInitialHandsReceived; //客户端收到的初始手牌计数器
...
=======
	void SortCards(CObList&);         // 给牌排序
	void SendCardsPacket(CObList& cardList, int forPlayerIndex);//发牌
>>>>>>> Stashed changes
...
=======
	...
	void BroadcastPacket(GamePacket* packet, ClientSocket* pExcludeClient=nullptr);//服务器广播
	void SendPacket(GamePacket* packet);//发送数据包
	void DealCardsAndBroadcast();
	void OnReceivePacket(GamePacket* packet, ClientSocket* pFromSocket);//接受数据包
	void ClearRound();   //一轮回合结束后清理？
private:
	Login* m_pSetupDlg; // << 新增 >> 指向当前活动的设置对话框
>>>>>>> Stashed changes
};
